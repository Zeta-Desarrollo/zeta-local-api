...
async function JSPDF (body, params){
    ...
    try{
        ...
        for (const ItemCode of body.products){
            ...
            const refWhiteFile = fs.readFileSync("./public/ref-no-ring.png")
...
async function noPriceLabel (body, params){
    let e
    let product = "lol"
    global.window = {document: {createElementNS: () => {return {}} }};
    global.navigator = {};
    ...
    try{
        ...
        for (const ItemCode of body.products){
            ...
            const qrFile = fs.readFileSync("./public/"+product.ItemCode+".png")
            const qr = new Uint8Array(qrFile);
            doc.addImage(qr, "PNG", leftEdge, 1, 5, 5)
            // doc.addImage(refWhite, "PNG", 3.6, 2.5, 1.3, 1.3)
            //qr s

            const logoFile = fs.readFileSync("./public/zeta-negro.png")
            const logo = new Uint8Array(logoFile);
            doc.addImage(logo, "PNG", 8 , 4.2, 4.14, 1.2)
            ...
            doc.setFillColor("#000000")
            // doc.setTextColor("#ffffff")
            doc.text(product.ItemCode, rightEdge, 1, "right")
            doc.setFontSize(16)

            let marcaText = product.FirmCode != -1? product.FirmName : ''
            // let line2 = doc.splitTextToSize(marcaText, rightEdge - leftEdge - leftSpace -4)
            FS=14
            doc.setFontSize(FS)
            ...
            //     FS-=0.1
            //     doc.setFontSize(FS)
            //     line2 = doc.splitTextToSize(marcaText, rightEdge - leftEdge - leftSpace -4)
            // }
            doc.text(marcaText, leftEdge+0.5, 1, "left")
