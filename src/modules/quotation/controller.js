...
const controller = {
    createQuotation: async({client, seller, priceList, products, quantities}, params)=>{
      ...
      await sqlPromise(sqliteDB, "run", `insert into quotation values (${quotation.Quotation}, '${quotation.client}','${quotation.client}', ${quotation.priceList}, ${quotation.created}, ${quotation.updated}, ${quotation.generated})`)
      for(const ItemCode of products){
        ...
        await sqlPromise(sqliteDB, "run", `insert into quotation_product values (${quotation.Quotation}, '${ItemCode}',${productData[ItemCode].Price}, '${productData[ItemCode].TaxCodeAR}', ${quantities[ItemCode]})`)
    ...
    list:async(body,params)=>{
      ...
      let quotations
      try{
        const data = await sqlPromise(sqliteDB, "all", "select * from quotation inner join quotation_product on quotation.Quotation=quotation_product.Quotation order by created desc")
        quotations = data
      }catch(e){
